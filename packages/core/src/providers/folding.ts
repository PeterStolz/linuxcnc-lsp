...
/** Fold each O-word block (sub/if/while/do/repeat) plus runs of >=2 comment
 *  lines in a G-code document. */
export function gcodeFoldingRanges(program: GcodeProgram, lineIndex: LineIndex): FoldingRange[] {
  const ranges: FoldingRange[] = [];
  for (const blk of program.blocks) {
    if (blk.endLine > blk.startLine) {
      ranges.push({ startLine: blk.startLine, endLine: blk.endLine, kind: FoldingRangeKind.Region });
    }
  }
  const isComment = (l: number) => /^\s*(;|\()/.test(lineIndex.lineText(l));
  let runStart = -1;
